...
io.on('connection', (socket) => {
  ...
  // Handle symbol subscription for real-time updates
  ...
  // Handle pattern history requests
  socket.on('getPatterns', async (data, callback) => {
    try {
      const { symbol, timeframe, limit } = data;
      console.log(`📊 Pattern history requested for ${symbol} (${timeframe})`);
      ...
      if (marketData && marketData.length >= 50) {
        ...
        console.log(`✅ Returned ${patterns.length} patterns for ${symbol}`);
      } else {
        if (callback) {
          callback({
            success: false,
  ...
  socket.on('updateSettings', (settings) => {
    console.log(`⚙️  User ${socket.id} updated settings:`, settings);
  });
...
const broadcastPatternDetected = (pattern) => {
  io.to(`patterns_${pattern.symbol}`).emit('newPattern', {
    ...pattern,
    timestamp: new Date().toISOString()
  });
  
  console.log(`📢 Broadcasted new pattern: ${pattern.type} on ${pattern.symbol}`);
};
              ...
              if (patterns && patterns.length > 0) {
                patterns.forEach(pattern => {
                  ...
                  if (pattern.status === 'completed' && pattern.confidence > 0.7) {
                    ...
                    console.log(`🎯 New ${pattern.type} pattern: ${symbol} (${timeframe}) - ${(pattern.confidence * 100).toFixed(1)}%`);
                  }
                });
              }
            }
...
const startServer = async () => {
  try {
    ...
    server.listen(PORT, () => {
      ...
      console.log(`📊 Market data: ${marketDataService.isSimulation ? 'Simulated' : 'Real'}`);
      console.log(`🔍 Pattern detection: ${process.env.PATTERN_DETECTION_INTERVAL || 30000}ms intervals`);
      console.log(`💬 WebSocket: Enabled for real-time updates`);
      console.log(`🤖 Telegram: ${telegramBot && telegramBot.isEnabled() ? 'Enabled' : 'Disabled'}`);
      console.log(`\n📈 Ready to detect harmonic patterns!\n`);
    });
